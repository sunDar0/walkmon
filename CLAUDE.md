...
| 파일 | 역할 | 담당 스킬 / 에이전트 |
|------|------|----------------------|
| `src/grid.js` | H3 좌표→셀 키 변환, 주변 셀 타일링·경로 보간 (플레이 뷰 `H3_RESOLUTION`=11 ≈ 50m; 전체 지도는 `FULLMAP_RES`=6 으로 롤업) | game-mechanics / game-core-engineer |
| `src/game.js` | 밸런스 상수·성장 (쿨다운 1h, 이동=AP 적립, 케어→크리처 XP, 미터 3종 감소·건강코드, 단계별 만렙·수동 진화, 레벨당 30XP) | game-mechanics / game-core-engineer |
| `src/occupy.js` | 점령·케어 상태 전이 순수 함수(`applyPath`·`tickState`·`careAction`·`treat`·`evolve`; `applyVisit` 은 얇은 래퍼). 저장 SSOT·포그라운드·백그라운드 공유 | game-mechanics / game-core-engineer |
| `src/items.js` | 지역 테마 아이템 드롭 | game-mechanics / game-core-engineer |
| `src/useLocation.js` | 포그라운드 위치 추적(`watchPositionAsync`, 10m/3s) | location-tracking / game-core-engineer |
| `src/backgroundLocation.js` | 백그라운드 추적(expo-task-manager). **App 마운트 시 `registerBackgroundLocation()` 로 연결됨**(포그라운드 active 면 배치 skip → 단일 writer) | location-tracking / game-core-engineer |
| `src/PixelHexMap.js` | 플레이 뷰: Skia 도트 타일맵·펫 스프라이트·카메라·케어 파티클 | pixel-rendering / pixel-render-engineer |
| `src/CareRoom.js` | 케어 방 뷰: 절차적 정육면체 방 + 크리처 산책·이모지 신호·파티클 | pixel-rendering / pixel-render-engineer |
| `src/FullMap.js` | 전체 지도 뷰: 만화풍 남한 실루엣(`koreaOutline.js`) 위 점령 셀 롤업 렌더 | pixel-rendering / pixel-render-engineer |
| `src/GameMap.js` | ~~네이티브 지도 헥스 렌더(react-native-maps)~~ **미사용(레거시)** — PixelHexMap 으로 대체, 어디서도 임포트 안 됨 | pixel-rendering / pixel-render-engineer |

상세 설계는 `walkmon_concept_plan.md`, 개요는 `README.md` 참고. 밸런스 수치를 바꿀 때는 이 표가 아니라 `src/game.js` 가 단일 출처.

## 핵심 함정

- **웹 분기**: `react-native-maps`·`react-native-skia` 는 웹 미지원. 네이티브 시각 컴포넌트마다 `.web.js` 짝이 따로 있다(PixelHexMap·CareRoom·FullMap). 네이티브 파일만 고치고 웹 짝을 빠뜨리지 말 것. (전체 규약은 walkmon-dev SKILL 의 "공유 계약 SSOT".)
...
| 2026-07-13 | 코드 지도 drift 동기화 — grid res11(50m)+전체지도 롤업, game.js 케어/AP 모델, occupy.js 진입점(applyPath·tickState·careAction·treat·evolve), backgroundLocation App 연결됨, CareRoom·FullMap 행 추가, GameMap 레거시(미사용) 표기, 웹 짝 예시에서 GameMap 제거 | CLAUDE.md | 케어 모델 전환·백그라운드 연결 후 코드 지도가 옛 XP 모델/미연결/누락 파일을 가리켜 오독 위험 |
